...
BlokusPiece iPiece = BlokusPiece(getRotations(iShape), false, "iPiece", 5);
...
BlokusPiece bigCornerPiece = BlokusPiece(getRotations(bigCornerShape), false, "bigCornerPiece", 5);
...
BlokusPiece crossPiece = BlokusPiece(getRotations(crossShape), false, "crossPiece", 5);
...
BlokusPiece snakePiece = BlokusPiece(getRotations(snakeShape), false, "snakePiece", 5);
...
std::unordered_map<blokusShapeType, std::vector<Matrix>> piecesMap; 


Matrix rotatePiece(Matrix p){
    std::vector<std::vector<bool>> rotated(p[0].size(), std::vector<bool>(p.size()));

    for(size_t row = 0 ; row < p[0].size() ; row++){
        for(size_t col = 0 ; col < p.size() ; col++){
            rotated[p[0].size()-1-col][row] = p[row][col];
        }
    }
    return rotated;
}
...
std::vector<Matrix> getRotations(Matrix p){
    std::vector<Matrix> rotations;
    for(int i = 0 ; i < 4 ; i++){
        Matrix rotation = rotatePiece(p);
...
void initializePieceMap() {
    piecesMap.insert(std::make_pair(blokusShapeType::iShapeType, getRotations(iShape)));
    piecesMap.insert(std::make_pair(blokusShapeType::smallCornerShapeType, getRotations(smallCornerShape)));
    piecesMap.insert(std::make_pair(blokusShapeType::bigCornerShapeType,  getRotations(bigCornerShape)));
    piecesMap.insert(std::make_pair(blokusShapeType::singleShapeType, getRotations(singleShape)));
